...
penalty_df <- pbp |>
  filter(event_type == "PENALTY",
         strength_state == "5v5") |> 
  select(event_type, secondary_type, event_player_2_type, event_player_2_name, event_player_2_id, strength_state) |> 
  drop_na(event_player_2_name) |> 
  rename(pen_type = secondary_type,
         player = event_player_2_name,
         player_id = event_player_2_id)

penalty_df

player_pen_draw <- penalty_df |> 
  count(player, player_id, sort = T, name = "penalties_drawn")
...
reg_data <- player_pen_draw |> 
  left_join(player_metrics, by = c("player")) |> 
...
reg_data |> 
  ...
  geom_jitter(alpha = .3) +
  geom_smooth() +
  facet_wrap(vars(name, position), scales = "free", ncol = 3)

lm_model <- lm(penalties_drawn ~ shots + position + goals + xg_median + shot_distance_median, data = reg_data)
